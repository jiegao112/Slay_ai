...
DEFAULT_IMAGE_DIR = PROJECT_ROOT / "img" / "yaoshui"
...
1. 用中文输出。
2. 忽略背景影响，只关注药水瓶主体。
3. 描述主体的外观、颜色、形状、液体或玻璃质感、瓶塞、标签和显著特征。
...
def update_miaoshu(connection: pymysql.Connection, name: str, miaoshu: str) -> int:
    with connection.cursor() as cursor:
        ...
        affected = cursor.execute(
            "UPDATE yaoshui SET miaoshu = %s WHERE name = %s",
            (miaoshu, name),
        )
    return affected
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="使用 Qwen 多模态模型描述药水图片，并写入 yaoshui.miaoshu。")
    parser.add_argument("--image-dir", type=Path, default=DEFAULT_IMAGE_DIR, help="图片目录，默认 img/yaoshui。")
...
def main() -> None:
    ...
    print(f"[INFO] 共发现 {len(image_paths)} 张药水图片。")
    if not image_paths:
        return
