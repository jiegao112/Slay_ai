...
COLLECTION_URL = f"{BASE_URL}/wiki/%E8%8D%AF%E6%B0%B4"
BMAX_BY_A = [20, 12, 10]

PROJECT_ROOT = Path(__file__).resolve().parents[1]
IMAGE_DIR = PROJECT_ROOT / "img" / "yaoshui"
...
def safe_filename(name: str, fallback: str = "yaoshui") -> str:
...
def extract_yaoshui_names(session: requests.Session, timeout: int) -> list[str]:
    ...
    for a_index, bmax in enumerate(BMAX_BY_A, start=1):
        for b_index in range(1, bmax + 1):
            ...
            if node is None:
                print(f"[WARN] 未找到药水名称: a={a_index}, b={b_index}")
                continue

            name = clean_text(node.text_content())
            if not name:
                print(f"[WARN] 药水名称为空: a={a_index}, b={b_index}")
                continue
...
def extract_image_url(tree: html.HtmlElement, page_url: str) -> str | None:
    node = xpath_first(tree, '//*[@id="mw-content-text"]/div/table[1]/tbody/tr[3]/td/img')
    if node is None:
        return None
...
def crawl_yaoshui_records(
    session: requests.Session,
    yaoshui: Iterable[str],
    timeout: int,
    sleep_seconds: float,
) -> list[YaoshuiRecord]:
    records: list[YaoshuiRecord] = []

    for index, name in enumerate(yaoshui, start=1):
        ...
        try:
            ...
            records.append(YaoshuiRecord(name=name, xiaoguo="", image_path=None))
            continue

        records.append(YaoshuiRecord(name=name, xiaoguo=xiaoguo, image_path=image_path))
...
def save_records_to_mysql(records: list[YaoshuiRecord]) -> None:
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                """
                CREATE TABLE IF NOT EXISTS yaoshui (
            ...
            cursor.executemany(
                """
                INSERT INTO yaoshui (name, xiaoguo, miaoshu)
                VALUES (%s, %s, %s)
                """,
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="爬取杀戮尖塔药水数据并写入 MySQL。")
    ...
    parser.add_argument("--limit", type=int, default=0, help="只处理前 N 个药水；0 表示全量。")
    parser.add_argument(
        "--cookie",
...
def main() -> None:
    ...
    try:
        yaoshui = extract_yaoshui_names(session, args.timeout)
    except (requests.exceptions.RequestException, RuntimeError) as exc:
        raise SystemExit(f"[ERROR] 药水收藏页抓取失败: {exc}") from exc

    if args.limit > 0:
        yaoshui = yaoshui[: args.limit]

    print(f"[INFO] 共抓取到 {len(yaoshui)} 个药水名称。")

    records = crawl_yaoshui_records(session, yaoshui, args.timeout, args.sleep)
    ...
    print("[INFO] 已写入 MySQL 表 yaoshui。")
