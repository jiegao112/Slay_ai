...
DEFAULT_IMAGE_DIR = PROJECT_ROOT / "img" / "guaiwu"
...
1. 用中文输出。
2. 忽略背景影响，只关注怪物主体。
3. 描述主体的外观、颜色、体型、姿态、材质感、肢体、武器或显著特征。
4. 输出一段 30 到 80 字的客观描述。"""
...
def build_llm() -> ChatOpenAI:
    ...
    if not api_key:
        raise RuntimeError("Missing env var QUEN_API.")
    if not base_url:
        raise RuntimeError("Missing env var QWEN_BASE_URL.")
...
def update_miaoshu(connection: pymysql.Connection, name: str, miaoshu: str) -> int:
    with connection.cursor() as cursor:
        ...
        affected = cursor.execute(
            "UPDATE guaiwu SET miaoshu = %s WHERE name = %s",
            (miaoshu, name),
        )
    return affected
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Describe enemy images with Qwen and write guaiwu.miaoshu.")
    parser.add_argument("--image-dir", type=Path, default=DEFAULT_IMAGE_DIR, help="Image directory, default img/guaiwu.")
    parser.add_argument("--limit", type=int, default=0, help="Only process first N images; 0 means all.")
    parser.add_argument("--dry-run", action="store_true", help="Generate descriptions without writing MySQL.")
...
def main() -> None:
    ...
    print(f"[INFO] Found {len(image_paths)} enemy images.")
    if not image_paths:
        return
    ...
    try:
        for index, image_path in enumerate(image_paths, start=1):
            ...
            print(f"[INFO] ({index}/{len(image_paths)}) Describing: {name}")
            ...
            if affected:
                print(f"[INFO] Updated database: {name}")
            else:
                print(f"[WARN] No database row matched name={name}; skipped.")
    finally:
